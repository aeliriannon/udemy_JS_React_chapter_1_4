...
let arrayGameList = Object.entries(progressGame)
      .filter(item => item[0] === 'firstPersone'|| item[0] === 'secondPerson'),
      currentArray = randomGenerator(arrayGameList),
      currentCity = randomGenerator(currentArray[1]),
      startGame = () => `${currentArray[0]}: ${currentCity}`;
...
function changePerson(array) {
    if (array[0] == 'firstPersone') { 
       return (array = arrayGameList[1]); 
    } else {
        return (array = arrayGameList[0]);  
    }
}

function definitionCurrentCity(cWord) { 
    let lastLetter = cWord[cWord.length - 1]; 
    currentCity = changePerson(currentArray)[1]
            .filter(item => item[0] === lastLetter)[0];
    return currentCity;
}

console.log(currentArray);
console.log(startGame());
console.log(changePerson(currentArray)[1]);
console.log(currentCity);
console.log(definitionCurrentCity(currentCity));
