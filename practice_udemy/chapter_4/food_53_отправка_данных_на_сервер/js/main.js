...
// Отпаравука данных на сервер Forms
    ...
    const message = { //создаем объект с ответами на нашу олтправку формы
        loading: 'Загрузка', //пока идет заргрузка формы
    ...
    function postData(form) { //Функция которая будет отправлять данные на сервер, как аргумент выступает форма для отправки
        form.addEventListener('submit', (e) => { //навешиваем на форму обработчик события сабмит, оно срабатывает при попытке отправить форму
            ...
            const statusMessage = document.createElement('div'); //сщздаем новый элекмент на страницек для вывода статус-сообщения по отправке формы
            statusMessage.classList.add('status'); //добавляем нашему новому элементу класс
            statusMessage.textContent = message.loading; //добавляем текст внутрь нового элемента о том что началась загрузка формы
...
//---!! !!---
            // request.setRequestHeader('Content-type', 'multipart/form-data'); //создаем заголовки запроса, что нам будет приходить
//---!! Когда используется FormData - конструктор, то заголовки к запросу прописывать не нужно !!---
           ...
           const formData = new FormData(form); //формируем новый объект, с помощью конструктора formData который возьмет все данные пользователя из формы
            ...
            request.send(formData); //отправляем нашу объект, который мы сформировали с помощью конструктора FormData, на сервер
            ...
            request.addEventListener('load', () => { //на нашу отправку вешаем обработчик события и отслеживаем полную загрузку формы
                if (request.status === 200) { //проверяем что статус отправки положительный, все хорошо
                    ...
                    statusMessage.textContent = message.success; //меняем текст в статус сообщении с загрузки на успешную отправку
                    form.reset(); //чистим данные формы, которые ввели
                    setTimeout(() => { //обращаемся  к setTimout
                        statusMessage.remove(); //и удаляем блок с сообщением-статуса
                    }, 2000); //через 2 секунды после его появления
                } else {
                    statusMessage.textContent = message.failure; //при неудачной загрузке меняем статус-сообщение на неудачу
                }
            });
        });
